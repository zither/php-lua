...
*/
static zval* php_lua_write_property(zval *object, zval *member, zval *value, void ** key) {
	php_lua_object * p_lua_object  = Z_LUAVAL_P(object);
	lua_State *L = p_lua_object->L;
	zend_string *str_member = zval_get_string(member);

#if (LUA_VERSION_NUM < 502)
	php_lua_send_zval_to_lua(p_lua_object, member);
...
/* }}} */
#else
/** {{{ static zval * php_lua_read_property(zend_object *object, zend_string *member, int type)
*/
zval *php_lua_read_property(zend_object *object, zend_string *member, int type, void **cache_slot, zval *rv){
	zval temp_object;
	php_lua_object *p_lua_object = php_lua_obj_from_obj(object);
	lua_State *L = p_lua_object->L;
	...
	ZVAL_OBJ(&temp_object, object);
	php_lua_get_zval_from_lua(L, -1, &temp_object, rv);
	lua_pop(L, 1);
...
/** {{{ static void php_lua_write_property(zend_object *object, zend_string *member, zval *value)
*/
static zval* php_lua_write_property(zend_object *object, zend_string *member, zval *value, void ** key) {
	zval temp_member;
	php_lua_object *p_lua_object = php_lua_obj_from_obj(object);
	lua_State *L = p_lua_object->L;
	ZVAL_STR(&temp_member, member);

#if (LUA_VERSION_NUM < 502)
	php_lua_send_zval_to_lua(p_lua_object, &temp_member);
	php_lua_send_zval_to_lua(p_lua_object, value);
...
static int php_lua_call_callback(lua_State *L) {
	...
	if (!zend_is_callable(func, 0, NULL)) {
		...
		php_lua_send_zval_to_lua(p_lua_object, &retval);
...
int php_lua_send_zval_to_lua(php_lua_object *p_lua_object, zval *val) /* {{{ */ {
		...
		case IS_ARRAY:
			{
				if (zend_is_callable(val, 0, NULL)) {
					...
					ZEND_HASH_FOREACH_KEY_VAL_IND(ht, longkey, key, v) {
						...
						if (!key_pushed) {
							php_lua_send_zval_to_lua(p_lua_object, &zkey);
						}

						php_lua_send_zval_to_lua(p_lua_object, v);
...
*/
static int php_lua_arg_apply_func(void *data, void *p_lua_object) {
	php_lua_send_zval_to_lua((php_lua_object *)p_lua_object, (zval*)data);
...
static zval *php_lua_call_lua_function(zval *lua_obj, zval *func, zval *args, int use_self, zval *retval) /* {{{ */ {
	...
	php_lua_object *p_lua_object = Z_LUAVAL_P(lua_obj);
	...
	if (args) {
		...
		zend_hash_apply_with_argument(Z_ARRVAL_P(args), (apply_func_arg_t)php_lua_arg_apply_func, (void *)p_lua_object);
	}
...
zend_function_entry lua_class_methods[] = {
	PHP_ME(lua, __construct,		arginfo_lua_void,  		ZEND_ACC_PUBLIC|ZEND_ACC_CTOR)
	...
	PHP_ME(lua, getVersion,			arginfo_lua_void, 		ZEND_ACC_PUBLIC)
